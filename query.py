...
class ChatSession:
    ...
    def _update_chat_history(self, user_query: str, response: str) -> None:
        ...
        self._chat_history.append(
            (get_uk_timestamp_utc_format(), "Next of Kin", user_query)
        )

        # Add response
        self._chat_history.append((get_uk_timestamp_utc_format(), "Florence", response))
...
def _fetch_and_parse_ehr(patient_number: str) -> list[str]:
    ...
    for paragraph in doc.paragraphs:
        if "Next of kin initiated a conversation with Florence." in paragraph.text:
            continue
